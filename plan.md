> **2026-09-26 当前授权：归一化 V1 seed44 小规模拟合检查。** 固定 seed42，从 Train/Validation 各分层抽取256题（Yes/No96、what96、where48、其余16），优先不同图像；Validation生成优先复用既有完整预测，Train使用无答案输入做贪心生成，并对两组做只读teacher-forced答案正文/模板CE。读取既有训练日志和诊断轨迹，不训练、不更新参数、不跑Test。输出样本清单、逐题CE/预测、重叠审计、分类型/长度/频次汇总及固定错误审阅包；先实现并提交推送，服务器诊断完成后据实追加两份账本，只提出一个优先行动建议并停止。
