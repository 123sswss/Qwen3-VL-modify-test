...
- [ ] GRASP纯问题查询版已跑出40.7254，但Prompt误放在视觉段之后，违背论文Eq.(6-7)，因此不得进入主表。论文顺序修正版现已实现并通过8项CPU单测：Prompt插在Qwen`<|vision_start|>`之前，形成`[Prompt, Visual, Question]`；待按纯问题Token、N=4、h=512、seed44和三轮协议完成最后一次PathVQA复现，不扫描`N/h/alpha`。
