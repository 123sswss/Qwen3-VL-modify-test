...
- 在任何Workspace结构大改之前，先运行最简SLAKE层消融`slake_dynamic_prompt_full_workspace_17only_seed44`：只将视觉锚点由5/11/17改为17，因此对应的私有视觉注入、Workspace更新Block和Workspace视觉读头都仅保留第17层；P20、私有Text CA256、S8/Visual CA128、Z32x1024、FFN4096、Workspace Text reader、全部学习率、seed44/data_seed42和3-epoch官方Test协议不变。参数审计固定为34,895,872；该实验用于判断浅/中层连续更新是否必要，不与同checkpoint关层干预混为一谈。
- 当前优先级改为先增效、后降本；暂停Workspace Block/Visual reader共享、FFN缩减和分支删除。下一项唯一训练变体为高容量`modality-separated Workspace`：保留P20、私有Text/Visual分支、Z32x1024、三套独立Block、FFN4096及全部读头，只将每层对`[full visual; full question]`的单个联合softmax CA替换为视觉CA与文本CA两个独立softmax分支，再用按slot学习的双路门控融合，门控初始化为等权。该控制直接消除约475视觉token对15文本token造成的约96.9%长度先验，不同时修改Text投影、槽数、宽度、FFN或学习率。
- 增效验证顺序：（1）先在PathVQA seed44固定epoch3 Validation与59.4504 Full Workspace作严格对照；（2）只有Overall或Free-form产生明确净增益，才原样迁移SLAKE seed44，重点观察当前仅+0.75且不显著的KVQA以及OPEN；（3）若双路CA有效，再单独把当前2560->1024单Linear文本投影升级为高容量MLP；（4）高分结构锁定并完成额外seed后，才恢复同checkpoint因果干预和参数共享/低秩化。不得在同一次实验中混入降参。
