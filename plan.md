...
- [ ] 先只运行seed45 epoch3 PathVQA Validation且不评估Test。若Overall低于56.9935（相对57.2935下降超过0.30）或Free-form低于23.4949（相对23.9949下降超过0.50），立即关闭本路线，不运行seed44。
- [ ] seed45通过单seed门槛后才运行seed44。两seed须同时满足：Overall均值不低于58.7350、分差不高于1.50、seed45必须高于57.2935，且Free-form均值不低于25.9518（原两seed均值26.4518最多下降0.50），才考虑补seed46。不得仅靠压低seed44制造低方差。
- [ ] 不扫描10%比例，不叠加Gate、正则、损失或其他初始化改动。服务器只在用户对该次运行明确授权后执行，且不自动关机；结果无论成功、失败或为负，均同步追加至两份实验账本。
