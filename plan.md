...
> **2026-09-23 V0 实施排期（用户已确认配置）：** 独立实现问题引导三层视觉选择＋共同原生 Value＋ADePT 风格真实问题偏移。固定 `r_q=128`、`r_delta=192`、三块各64、depthwise k3＋pointwise 残差卷积、P20＋索引17的S8/A_v10 Visual18。偏移输出层 `Normal(0,1e-4)`、零 bias；预计2,356,675个可训练参数，以实际模块审计为准。仅准备 **PathVQA seed44/data seed42、3 epochs、epoch3 Validation** 单次入口；不安排 Test、多 seed 或候选消融。服务器运行仍需用户针对该次明确授权。首个真实 batch 审计偏移/原问题 RMS、各分支首轮梯度、三层地图与层权重；完成运行后将结果同时写入两份实验账本。
