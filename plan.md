...
- SLAKE迁移已实现，待运行目标为`slake_dynamic_prompt_full_workspace_seed44`：复用同一Dynamic Prompt/Workspace模型与checkpoint格式，固定P20、Text CA256、私有视觉S8/CA128、视觉层5/11/17、Z32x1024、3个独立Workspace Block、FFN4096、共享读头CA1024和全部学习率；SLAKE train.json训练3 epochs后只评估epoch3官方test，seed44/data_seed42，预期参数审计76,896,256。该运行是跨数据集泛化检查，不允许根据SLAKE结果临时改超参。
