...
以当前 [paper_en.tex](E:/AAA24105033/project/py/visionOnly/Qwen3-VL-modify-test/els-cas-templates/paper_en.tex) 和 [body_en.tex](E:/AAA24105033/project/py/visionOnly/Qwen3-VL-modify-test/els-cas-templates/body_en.tex) 为准，按 EAAI/ESWA 等严肃应用 AI 期刊的门槛重构。SLAKE 和后续公开 benchmark 承担主要实验与统计证据；自建电力数据集只保留为一次性的真实应用案例、数据构建工作量和潜在公开贡献，不再追加任何训练、重复运行或分析实验。
...
| 4. 非参数统计 | SLAKE 和公开基线没有逐题显著性检验与置信区间；现有五次运行包含四个独立种子和一次 Seed 44 技术重复，需要准确区分 | 极高 |
| 5. 公平调参 | 只完整报告了 FROST-VL 参数；没有验证集和基线调参协议，容易被怀疑使用测试集选参 | 极高 |
| 6. 数据和划分 | 缺少验证集、类别分布、去重方法、标注一致性、生成题目流程和公开数据具体规模 | 极高 |
| 7. 应用分析 | 现有 98.40% Top-1 agreement 是很强的输出行为保持证据，但它只比较首个生成位置且没有任务正确答案，不能单独支撑“100% 保存原生通用能力”。自建电力数据不再承担进一步机制或统计证明，只作为真实应用展示 | 极高 |
...
### 1. 固定公开数据协议，消除测试集选参嫌疑

- 自建电力数据的现有训练、测试和结果全部冻结，不重新划分验证集，不补种子，不重新训练。正文只如实说明实际训练使用 20,000 个样本、完整训练池包含 20,015 张图像和 36,700 条 instruction、测试集包含 972 张独立图像及问题，并承认其私有性限制。
- 自建数据不用于后续超参数选择、统计显著性结论或跨方法稳定性主张；现有分数只作为一次真实应用案例的描述性结果。
...
- 后续多种子训练只在 SLAKE 和额外公开垂直 benchmark 上进行。自建数据现有 Single Adapter 32.70%、w/o Relation Loss 52.31% 等结果不再追加复验，也不作为跨种子稳定性证据。
- SLAKE 关键消融至少运行 3 个独立种子，以公开数据上的结果取代自建数据消融作为主要机制证据。

### 4. 非参数统计分析

- 在 SLAKE、额外公开垂直 benchmark 和 MMBench 的同一测试题上，先使用 Cochran’s Q 检验；显著后，以精确 McNemar 检验进行 FROST-VL 与各关键基线的成对比较。
- 多重比较使用 Holm 校正。
...
- 不再为自建电力数据新增多种子、显著性检验、题型分组、类别表、错误分析、门控评测、消融或新基线；也不增加 YOLO/mAP 等任务不一致的比较。
- 主文只保留数据构建统计、一个简洁的应用性能表和必要的定性案例，用于说明真实工作量与应用可行性。避免让私有数据在多个实验表中反复承担核心结论。
- 现有自建数据消融表在 SLAKE 消融完成前暂时保留；公开消融齐全后，将其移至附录或删去重复项，主文以 SLAKE 消融为准。
- 门控器、困难负样本、内部机制、统计稳定性和方法比较全部改在 SLAKE、额外公开垂直 benchmark、LLaVA 或 MMBench 上完成。
...
- SLAKE 和其他承担主要结论的公开 benchmark 有配对非参数检验、置信区间和逐种子结果；自建数据明确标注为单次描述性应用结果，不参与统计外推。
...
- [x] **冻结自建数据实验**：保留现有训练结果作为一次性真实应用案例，不再追加种子、基线、统计、消融或分类分析；后续实验预算全部投入公开 benchmark。
- [ ] **额外公开垂直领域 benchmark**：选择一个图像、标注、许可证和评分脚本均可下载的非 SLAKE 专业 VQA 数据，只运行 Base、FROST-VL、最强同范围 PEFT 和 Full-VLM LoRA，用于验证跨领域适用性。
- [ ] **主要准确率的非参数统计**：保存逐题预测，对 SLAKE、额外公开垂直 benchmark 和 MMBench 进行 Cochran’s Q、McNemar、Holm 校正和配对 bootstrap 置信区间。
- [ ] **通用 VLM 能力 benchmark**：使用 MMBench EN dev 比较 Base、FROST-VL 和 Force \(G=1\)。如果决定不做，则必须把全文结论严格限制为“首 token 输出行为保持”，不能声称通用能力未下降。

### B. 强烈建议：用于证明方法不是模块堆叠

- [ ] **SLAKE 关键消融多种子实验**：在 SLAKE 上运行 Single Adapter、Homogeneous Adapter LR、w/o Rep Token 和 w/o Relation Loss；自建数据的 32.70% 和 52.31% 不再复验，只作为已有描述性结果。
- [ ] **SLAKE 其余损失和路由消融**：在 SLAKE 上补充 w/o Usage、w/o Entropy、w/o Effective-offset、w/o Dispersion Floor、视觉单模态 Router 和 Force \(G=1\)。
...
本节采用已经确定的“方法为主、公开实验为证据、私有数据为案例”路线。FROST-VL 不再被描述为电力专用方法，而是冻结 VLM 上的任务门控式低干扰外部适配框架。SLAKE 成为主要公开实验对象，后续增加一个通用能力 benchmark 和一个额外的公开垂直领域 benchmark。自建电力数据集只保留为一次性真实应用案例、潜在数据贡献和数据构建工作量，不再继续参与新实验。所有具体数字等公开实验锁定后再统一更新。
...
- [ ] **将四类数据的职责彻底分开**：
  - 自建电力电气数据集：一次性真实应用案例、数据构建工作量和未来可能公开的潜在贡献；保留现有结果，不再新增或重复实验；
...
- [ ] **确定四个研究问题并据此组织结果**：RQ1 在 SLAKE 和额外公开垂直 benchmark 上的领域能力提升；RQ2 输出行为和通用能力的低干扰保持；RQ3 Gate、Rep Token、Router、Adapter 和约束项在公开数据上的机制作用；RQ4 参数、缓存和推理开销。自建电力结果只作为 RQ1 的辅助应用案例，不承担主要统计结论。
...
- [ ] **摘要改成方法优先结构**：依次写冻结 VLM 的领域适配问题、低干扰外部支路、核心机制、SLAKE 与额外公开 benchmark 的主要结果、通用能力/行为保持，最后用一句话补充自建电力真实应用案例。电力不再出现在摘要第一句，也不把尚未公开的数据集称为公共贡献或主要统计证据。
- [ ] **Introduction 从通用问题开始**：先讨论专业领域适配为什么会影响共享路径和一般能力，再引出条件化外部支路；电力和医疗只作为两类代表性专业场景。删除“该结构源于电力设备特点”的暗示。
- [ ] **重写贡献列表**：第一项是任务门控的外部视觉偏移框架；第二项是 Rep-Token 专家偏移与多 Adapter 路由；第三项是低干扰约束和多个公开 benchmark 上的可复现实验。自建电力数据构建与应用评测只作为补充贡献，不承诺公开，也不与公开实验并列为主要方法证据。
...
- [ ] **Experiment 按证据职责重排**：先给统一协议和数据角色，再依次呈现 SLAKE 主实验、公开数据上的消融与内部诊断、额外公开垂直领域、通用能力与输出行为保持、推理效率，最后用一个简洁小节呈现自建电力应用案例。不同数据集不混在同一排名表中，自建数据不重复出现在多个新增实验表中。
...
- [ ] **压缩并澄清自建数据说明**：交代 20,015 张训练图像、36,700 条 instruction、实际抽取 20,000 条样本和 972 张独立测试图像之间的关系，以及现有去重和人工检查流程。不再为它新建验证集或追加实验；明确其结果是冻结的单次应用案例。
