...
1. **主模型采用 D768**：旧 `S8+A_v10` 版本 PathVQA Validation 为 59.5622；最终候选改为统一 `V20`，7.807M 可训练参数，待完成同 seed 新基线后冻结。
...
- no-static-visual 已证明静态视觉校准具有实质贡献，但旧 `S8+A_v10` 参数化不作为最终结构证据；统一 `V20` 必须先复现 Layer17 新基线。
...
7. 最终候选在 layer17 插入一张统一的 `V20 x 1024` 静态视觉 Prompt 表；`V20` 与 `Z10` 完全独立，动态 `Z -> visual` 写回已经删除。
...
| 优先级 | 实验 | Seed | 目的 |
|---|---|---:|---|
| P0 | D768 移除静态视觉插入 | 44 | 已完成：58.4438，显著下降1.1184；最终结构保留统一的20个Layer17静态视觉Prompt |
...
- [x] 实现 learned-static-query D768；最终统一 `V20` 参数化下总参数为 7,807,232。
