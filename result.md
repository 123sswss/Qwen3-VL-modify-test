...
|---|---:|---:|---:|---:|---|
| Dynamic Prompt + Sparse Visual, LR3e-5 | 44 | **57.51** | 89.62 | **25.35** | Current best PathVQA result; versus Dynamic Prompt +0.97 Overall and+2.14 Free-form with paired clustered CIs excluding zero. The only change from the failed high-LR control is Sparse LR3e-4 ->3e-5. |
...
- Storage note: the two completed dual-path Sparse Visual checkpoints were explicitly deleted on2026-08-28; logs and prediction-level evidence remain. The57.51 score is the new numerical reference, but the model weights are unavailable for Sparse-off intervention or deployment.
