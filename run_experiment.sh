...
run_qdpt_d768_final_dataset() {
  ...
  case "$variant" in
    question_static_visual)
      ...
      ;;
    question_static_visual_sandwich)
      experiment_stem="qdpt_d768_question_q10_l17_p20_s8_av10_sandwich"
      expected_trainable=7805184
      query_source="question_attention_pooling"
      static_visual_write=true
      private_visual_tokens=8
    ...
    question_only)
      ...
      control_flags=(
        --directional-query-source question_attention_pooling
        --directional-visual-conditioning question_only
        --directional-static-visual-write
      )
      ;;
    ...
    layer18_static_visual)
      experiment_stem="qdpt_d768_question_q10_l18_p20_s8_av10"
      expected_trainable=7805184
      query_source="question_attention_pooling"
      static_visual_write=true
      private_visual_tokens=8
    ...
    layers17_18_19_shared_static_visual)
      experiment_stem="qdpt_d768_question_q10_l17_18_19_shared_p20_s8_av10"
      expected_trainable=7805184
      query_source="question_attention_pooling"
      static_visual_write=true
      private_visual_tokens=8
  ...
  (
    ...
    python -m "$train_module" \
      ...
      --sparse-visual-rep-tokens "$visual_prompt_tokens_arg" \
      --sparse-visual-attention-dim 128 \
      --sparse-visual-heads 4 \
...
run_pathvqa_grasp_qwen_adapted_seed44() {
  local experiment_name="pathvqa_grasp_qwen_adapted_embedding_init_dual_lr_n4_h512_seed44"
  local output_dir
  output_dir="$(available_output_dir "$PATHVQA_GRASP_OUTPUT_ROOT" "${experiment_name}_${RUN_DATE}")"
  mkdir -p "$output_dir"
  echo "[PATHVQA_GRASP_QWEN_ADAPTED_CONFIG] experiment=$experiment_name seed=44 data_seed=42 blocks=4 bottleneck=512 alpha=1.5 prompt_tokens=1 prompt_init=embedding_rows prompt_lr=0.3 prompt_weight_decay=0 projection_lr=1e-4 projection_weight_decay=0.01 question=raw_question_only_frozen_llm_last_hidden_mean visual=post_merger_grid prompt_placement=before_visual_segment expected_trainable=2632704 epochs=3 split=validation output=$output_dir"
  (
    cd "$ROOT_DIR" || exit 1
    python -m unittest test_grasp_prompt_tuning.py || exit 1
    python -m pathvqa.train_grasp \
...
run_pathvqa_grasp_qwen_adapted_no_position_marathon10_lr009_seed44() {
  local experiment_name="pathvqa_grasp_qwen_adapted_no_position_marathon10_lr009_n4_h512_seed44"
  local output_dir
  output_dir="$(available_output_dir "$PATHVQA_GRASP_OUTPUT_ROOT" "${experiment_name}_${RUN_DATE}")"
  mkdir -p "$output_dir"
  echo "[PATHVQA_GRASP_QWEN_ADAPTED_MARATHON_LR009_CONFIG] experiment=$experiment_name seed=44 data_seed=42 blocks=4 bottleneck=512 alpha=1.5 prompt_tokens=1 prompt_init=embedding_rows prompt_lr=0.09 prompt_weight_decay=0 projection_lr=1e-4 projection_weight_decay=0.01 position_encoding=none question=raw_question_only_frozen_llm_last_hidden_mean visual=post_merger_grid prompt_placement=before_visual_segment expected_trainable=2632704 epochs=10 scheduler=linear scheduler_horizon_epochs=10 prompt_lr_area_matches_three_epoch_lr03=true finite_fail_fast=true full_validation_epochs=3,4,5,6,7,8,9,10 compact_progress=$output_dir/marathon_progress.tsv output=$output_dir"
  (
    cd "$ROOT_DIR" || exit 1
    python -m unittest test_grasp_prompt_tuning.py || exit 1
    python -m pathvqa.train_grasp \
  ...
  for epoch in $(seq 3 10); do
    ...
    if [ ! -f "$checkpoint/grasp_prompt.pt" ]; then
      printf '%s\t\t\t\t%s\tmissing_checkpoint\n' "$epoch" "$checkpoint" >> "$progress"
      echo "[PATHVQA_GRASP_MARATHON_LR009_EVAL] epoch=$epoch status=missing_checkpoint" >&2
      suite_failures=$((suite_failures + 1))
      continue
    fi
    python pathvqa/pathvqa_official_eval.py \
    ...
    if [ "$eval_status" -ne 0 ]; then
      printf '%s\t\t\t\t%s\tfailed\n' "$epoch" "$checkpoint" >> "$progress"
      echo "[PATHVQA_GRASP_MARATHON_LR009_EVAL] epoch=$epoch status=failed exit_code=$eval_status" >&2
      suite_failures=$((suite_failures + 1))
      continue
    fi
    python -c 'import json,sys; epoch,summary_path,checkpoint,progress=sys.argv[1:]; data=json.load(open(summary_path,encoding="utf-8")); row=(epoch,str(data["overall_accuracy"]),str(data["yes_no_accuracy"]),str(data["free_form_accuracy"]),checkpoint,"complete"); open(progress,"a",encoding="utf-8").write("\t".join(row)+"\n"); print("[PATHVQA_GRASP_MARATHON_LR009_EVAL] epoch="+epoch+" overall="+row[1]+" yes_no="+row[2]+" free_form="+row[3]+" status=complete")' \
      "$epoch" "$eval_dir/pathvqa_summary.json" "$checkpoint" "$progress" || {
        suite_failures=$((suite_failures + 1))
        continue
      }
  ...
  cat "$progress"
  python -c 'import csv,sys; rows=[r for r in csv.DictReader(open(sys.argv[1],encoding="utf-8"),delimiter="\t") if r["status"]=="complete"]; assert rows,"no completed GRASP marathon LR009 evaluations"; best=max(rows,key=lambda r:float(r["overall"])); print("[PATHVQA_GRASP_MARATHON_LR009_BEST] epoch="+best["epoch"]+" overall="+best["overall"]+" yes_no="+best["yes_no"]+" free_form="+best["free_form"]+" checkpoint="+best["checkpoint"])' "$progress" || return 1
  if [ "$suite_failures" -ne 0 ]; then
    echo "[ERR] PathVQA GRASP marathon LR009 evaluation failures=$suite_failures; all epochs were attempted." >&2
    return 1
  fi
}
...
run_slake_qdpt_d768_final_seeds44_46() {
  ...
  local run_seed
  for run_seed in 44 45 46; do
    echo "[SLAKE_QDPT_D768_FINAL] seed=$run_seed status=starting"
    if run_qdpt_d768_final_dataset \
      slake question_static_visual "$run_seed"; then
      echo "[SLAKE_QDPT_D768_FINAL] seed=$run_seed status=completed"
    else
      echo "[SLAKE_QDPT_D768_FINAL] seed=$run_seed status=failed_continue" >&2
      suite_failures=$((suite_failures + 1))
    fi
  done
  if [ "$suite_failures" -ne 0 ]; then
    echo "[ERR] SLAKE QDPT-D768 final suite failures=$suite_failures; all three seeds were attempted." >&2
    return 1
  fi
  echo "[SLAKE_QDPT_D768_FINAL_DONE] seeds=44,45,46 status=completed"
}
...
run_pathvqa_qdpt_paper_seed_stability_and_dynamics() {
  local bundle_dir="$ROOT_DIR/paper_figures/output/final_bundle_${RUN_DATE}"
  mkdir -p "$bundle_dir"
  echo "[QDPT_PAPER_FIGURES_5_6] training=false output=$bundle_dir"
  (
    cd "$ROOT_DIR" || exit 1
    python -m unittest test_qdpt_paper_figures.py || exit 1
    python -m paper_figures.qdpt_figures stability \
      --output "$bundle_dir/figure3_seed_stability" || exit 1
    python -m paper_figures.qdpt_figures dynamics \
      --run "seed44=$PATHVQA_DYNAMIC_PROMPT_OUTPUT_ROOT/pathvqa_qdpt_d768_question_q10_l17_p20_s8_av10_sandwich_seed44_20260909" \
      --run "seed45=$PATHVQA_DYNAMIC_PROMPT_OUTPUT_ROOT/pathvqa_qdpt_d768_question_q10_l17_p20_s8_av10_sandwich_seed45_20260910_1" \
      --run "seed46=$PATHVQA_DYNAMIC_PROMPT_OUTPUT_ROOT/pathvqa_qdpt_d768_question_q10_l17_p20_s8_av10_sandwich_seed46_20260910" \
...
run_pathvqa_v1_loss_corrected_seed44() {
  ...
  local original_v1_eval="$PATHVQA_V1_OUTPUT_ROOT/pathvqa_v1_visual_selection_prefix_p20_seed44_20260924_1/eval_validation/epoch_3"
  ...
  if [ ! -f "$original_v1_eval/pathvqa_comparisons.json" ]; then
    echo "[ERR] Original V1 paired predictions missing: $original_v1_eval; no training started." >&2
    return 1
  fi
  if ! python -c 'import importlib.metadata,json,sys,torch; d=json.load(open(sys.argv[1],encoding="utf-8")); assert d["passed"] is True and d["training_commit"]=="4636416ee99768c667377f0c66b5809daf48ffcd"; assert all(all(d["windows"][str(k)]["numeric_checks"].values()) for k in (16,3)); assert d["versions"]["torch"]==torch.__version__; assert d["versions"]["transformers"]==importlib.metadata.version("transformers"); assert d["versions"]["accelerate"]==importlib.metadata.version("accelerate"); print("[V1_NORM_FIXED_PREFLIGHT] audit_passed=True versions_match=True full16_and_tail3=True")' "$audit_json"; then
    echo "[ERR] V1 loss audit failed or runtime versions changed; rerun the read-only audit before training." >&2
    return 1
  fi
  local experiment_name="pathvqa_v1_visual_selection_prefix_p20_norm_fixed_seed44"
  local output_dir
  output_dir="$(available_output_dir "$PATHVQA_V1_OUTPUT_ROOT" "${experiment_name}_${RUN_DATE}")"
  mkdir -p "$output_dir/eval_validation/epoch_3"
  echo "[PATHVQA_V1_LOSS_CORRECTED_CONFIG] experiment=$experiment_name audit=$audit_json git_commit=$(git -C "$ROOT_DIR" rev-parse HEAD) model_seed=44 data_seed=42 batch=2 accumulation=16 epochs=3 output=$output_dir"
  (
    cd "$ROOT_DIR" || exit 1
    python -m pathvqa.train_visual_selection_prefix \
      --model-path "$MODEL_PATH" --data-root "$PATHVQA_DATA_ROOT" \
  ...
  (
    ...
    python -m diagnostics.compare_pathvqa_v1_baselines \
      --v1-eval "$output_dir/eval_validation/epoch_3" \
      --original-v1-eval "$original_v1_eval" \
      --v0-eval "$PATHVQA_V0_OUTPUT_ROOT/diagnostics/pathvqa_v0_seed44_mask_fixed_validation_20260923_1/mask_fixed_normal" \
      --cocoop-eval "$PATHVQA_COCOOP_OUTPUT_ROOT/pathvqa_cocoop_style_p20_h160_seed44_20260909/eval_validation/epoch_3" \
      --static-eval "$PATHVQA_PROMPT_OUTPUT_ROOT/pathvqa_prompt_tuning_len20_seed44_20260827/eval_validation/epoch_3" \
      --output "$output_dir/paired_baseline_comparison.json" \
...
run_pathvqa_v1_norm_fixed_5ep_seed45() {
  local audit_json="${PATHVQA_V1_LOSS_AUDIT_JSON:-$PATHVQA_V1_OUTPUT_ROOT/diagnostics/pathvqa_v1_loss_scaling_audit_20260924/v1_loss_scaling_audit.json}"
  local three_epoch_run="$PATHVQA_V1_OUTPUT_ROOT/pathvqa_v1_visual_selection_prefix_p20_norm_fixed_seed45_20260925"
  local three_epoch_eval="$three_epoch_run/eval_validation/epoch_3"
  local three_epoch_checkpoint="$three_epoch_run/checkpoints/epoch_3"
  local fixed_fit="$PATHVQA_V1_OUTPUT_ROOT/diagnostics/pathvqa_v1_norm_fixed_seed44_fit_audit_20260926"
  local sample_manifest="$fixed_fit/sample_manifest.json"
  if [ ! -f "$audit_json" ]; then
    echo "[ERR] V1 loss-scaling audit JSON missing: $audit_json; no training started." >&2
    return 1
  fi
  if ! python -c 'import importlib.metadata,json,sys,torch; d=json.load(open(sys.argv[1],encoding="utf-8")); assert d["passed"] is True and d["training_commit"]=="4636416ee99768c667377f0c66b5809daf48ffcd"; assert all(all(d["windows"][str(k)]["numeric_checks"].values()) for k in (16,3)); assert d["versions"]["torch"]==torch.__version__; assert d["versions"]["transformers"]==importlib.metadata.version("transformers"); assert d["versions"]["accelerate"]==importlib.metadata.version("accelerate"); print("[V1_5EP_SEED45_NORM_PREFLIGHT] audit_passed=True versions_match=True full16_and_tail3=True")' "$audit_json"; then
    echo "[ERR] V1 loss audit failed or runtime versions changed; no training started." >&2
    return 1
  fi
  if [ ! -f "$three_epoch_checkpoint/visual_selection_prefix.pt" ] \
     || [ ! -f "$three_epoch_eval/pathvqa_comparisons.json" ] \
     || [ ! -f "$sample_manifest" ]; then
    echo "[ERR] Seed45 epoch3 checkpoint/predictions or fixed fitting sample are missing; no training started." >&2
    return 1
  fi
  if ! python -c 'import hashlib,sys; p=sys.argv[1]; digest=hashlib.sha256(open(p,"rb").read()).hexdigest(); print(f"[V1_5EP_SEED45_SAMPLE_MANIFEST] path={p} sha256={digest}"); assert digest=="5ba0ae685ff6570e509f37f93a7990f4050a848d96883ca001082fa27dfda930"' "$sample_manifest"; then
    echo "[ERR] Fixed fitting manifest identity changed; no training started." >&2
    return 1
  fi
  local experiment_name="pathvqa_v1_norm_fixed_5ep_seed45"
  local output_dir
  output_dir="$(available_output_dir "$PATHVQA_V1_OUTPUT_ROOT" "${experiment_name}_${RUN_DATE}")"
  mkdir -p "$output_dir/eval_validation/epoch_5"
  echo "[PATHVQA_V1_5EP_SEED45_CONFIG] experiment=$experiment_name git_commit=$(git -C "$ROOT_DIR" rev-parse HEAD) model_seed=45 data_seed=42 batch=2 accumulation=16 epochs=5 save_epochs=3,4,5 primary_epoch=5 warmup_ratio=0.03 scheduler=linear_to_epoch5 model_accepts_loss_kwargs=false output=$output_dir"
  (
    cd "$ROOT_DIR" || exit 1
    python -m pathvqa.train_visual_selection_prefix \
      --model-path "$MODEL_PATH" --data-root "$PATHVQA_DATA_ROOT" \
      --output-dir "$output_dir" --experiment-name "$experiment_name" \
      --model-seed 45 --epochs 5 --save-epochs 3 4 5 \
      --correct-loss-accumulation \
      2>&1 | tee "$output_dir/train.log"
  ) || return 1
  local epoch
  for epoch in 3 4 5; do
    [ -f "$output_dir/checkpoints/epoch_${epoch}/visual_selection_prefix.pt" ] \
      || { echo "[ERR] V1 seed45 5ep requested checkpoint missing: epoch_$epoch" >&2; return 1; }
  done
  if ! python -c 'import json,sys; d=json.load(open(sys.argv[1],encoding="utf-8")); assert d["model_seed"]==45 and d["data_seed"]==42; assert d["epochs"]==5 and d["saved_epochs"]==[3,4,5]; assert d["trainer_model_accepts_loss_kwargs"] is False; assert d["optimizer"]["warmup_ratio"]==0.03 and d["optimizer"]["scheduler"]=="linear"; print("[V1_5EP_SEED45_TRAIN_REPORT_AUDIT] model_seed=45 data_seed=42 epochs=5 saved_epochs=3,4,5 normalized=True scheduler=linear warmup_ratio=0.03")' "$output_dir/train_report.json"; then
    echo "[ERR] V1 seed45 5ep train report does not attest the requested protocol." >&2
    return 1
  fi
  local checkpoint="$output_dir/checkpoints/epoch_5"
  (
  ...
  (
    ...
    python -m diagnostics.compare_pathvqa_v1_training_budget \
      ...
      --baseline pathvqa_v1_visual_selection_prefix_p20_norm_fixed_seed45 \
      --output "$output_dir/paired_5ep_vs_3ep.json" \
      2>&1 | tee "$output_dir/paired_5ep_vs_3ep.log"
  ) || return 1
  (
    cd "$ROOT_DIR" || exit 1
    python -m diagnostics.diagnose_pathvqa_v1_fit \
      --model-path "$MODEL_PATH" --checkpoint "$three_epoch_checkpoint" \
      --data-root "$PATHVQA_DATA_ROOT" --cache-dir "$PATHVQA_CACHE_ROOT" \
      --validation-eval "$three_epoch_eval" \
      --train-run-root "$three_epoch_run" --sample-manifest "$sample_manifest" \
      --output-dir "$output_dir/fit_audit_epoch_3_reference" \
      2>&1 | tee "$output_dir/fit_audit_epoch_3_reference.log"
    python -m diagnostics.diagnose_pathvqa_v1_fit \
      --model-path "$MODEL_PATH" --checkpoint "$checkpoint" \
      --data-root "$PATHVQA_DATA_ROOT" --cache-dir "$PATHVQA_CACHE_ROOT" \
      --validation-eval "$output_dir/eval_validation/epoch_5" \
      ...
      2>&1 | tee "$output_dir/fit_audit_epoch_5.log"
    python -m diagnostics.compare_pathvqa_v1_fit_audits \
      --three-epoch-audit "$output_dir/fit_audit_epoch_3_reference/fit_audit.json" \
      --five-epoch-audit "$output_dir/fit_audit_epoch_5/fit_audit.json" \
      --experiment "$experiment_name" \
      --output "$output_dir/fit_audit_5ep_vs_3ep.json" \
      2>&1 | tee "$output_dir/fit_audit_5ep_vs_3ep.log"
  ...
  score="$(python -c 'import json,sys;print(json.load(open(sys.argv[1],encoding="utf-8"))["overall_accuracy"])' "$output_dir/eval_validation/epoch_5/pathvqa_summary.json")" || return 1
  printf 'experiment\tseed\tprotocol\tvalidation_epoch\tvalidation_accuracy\tcheckpoint\n' > "$output_dir/selected_result.tsv"
  printf '%s\t45\tfixed_epoch5_validation\t5\t%s\t%s\n' "$experiment_name" "$score" "$checkpoint" >> "$output_dir/selected_result.tsv"
  cat "$output_dir/selected_result.tsv"
  echo "[PATHVQA_V1_5EP_SEED45_DONE] output=$output_dir primary_epoch=5 test_evaluation=false other_seeds=false"
}
...
run_pathvqa_v1_loss_corrected_seed45() {
  ...
  local experiment_name="pathvqa_v1_visual_selection_prefix_p20_norm_fixed_seed45"
  local output_dir
  output_dir="$(available_output_dir "$PATHVQA_V1_OUTPUT_ROOT" "${experiment_name}_${RUN_DATE}")"
  mkdir -p "$output_dir/eval_validation/epoch_3"
  echo "[PATHVQA_V1_LOSS_CORRECTED_CONFIG] experiment=$experiment_name audit=$audit_json git_commit=$(git -C "$ROOT_DIR" rev-parse HEAD) model_seed=45 data_seed=42 batch=2 accumulation=16 epochs=3 output=$output_dir"
  (
    cd "$ROOT_DIR" || exit 1
    python -m pathvqa.train_visual_selection_prefix \
      --model-path "$MODEL_PATH" --data-root "$PATHVQA_DATA_ROOT" \
      --output-dir "$output_dir" --experiment-name "$experiment_name" \
      --model-seed 45 --correct-loss-accumulation \
      2>&1 | tee "$output_dir/train.log"
  ) || return 1
  local checkpoint="$output_dir/checkpoints/epoch_3"
  [ -f "$checkpoint/visual_selection_prefix.pt" ] || return 1
...
run_pathvqa_v1_loss_corrected_seed46() {
  local audit_json="${PATHVQA_V1_LOSS_AUDIT_JSON:-$PATHVQA_V1_OUTPUT_ROOT/diagnostics/pathvqa_v1_loss_scaling_audit_20260924/v1_loss_scaling_audit.json}"
  if [ ! -f "$audit_json" ]; then
    echo "[ERR] V1 loss-scaling audit JSON missing: $audit_json; no training started." >&2
    return 1
  ...
  local experiment_name="pathvqa_v1_visual_selection_prefix_p20_norm_fixed_seed46"
  local output_dir
  output_dir="$(available_output_dir "$PATHVQA_V1_OUTPUT_ROOT" "${experiment_name}_${RUN_DATE}")"
  mkdir -p "$output_dir/eval_validation/epoch_3"
  echo "[PATHVQA_V1_LOSS_CORRECTED_CONFIG] experiment=$experiment_name audit=$audit_json git_commit=$(git -C "$ROOT_DIR" rev-parse HEAD) model_seed=46 data_seed=42 batch=2 accumulation=16 epochs=3 output=$output_dir"
  (
    cd "$ROOT_DIR" || exit 1
    python -m pathvqa.train_visual_selection_prefix \
      --model-path "$MODEL_PATH" --data-root "$PATHVQA_DATA_ROOT" \
      --output-dir "$output_dir" --experiment-name "$experiment_name" \
      --model-seed 46 --correct-loss-accumulation \
      2>&1 | tee "$output_dir/train.log"
  ) || return 1
  local checkpoint="$output_dir/checkpoints/epoch_3"
  [ -f "$checkpoint/visual_selection_prefix.pt" ] || return 1
...
run_pathvqa_v3_postvisual_prefix_p20_norm_fixed_seed44() {
  local audit_json="${PATHVQA_V1_LOSS_AUDIT_JSON:-$PATHVQA_V1_OUTPUT_ROOT/diagnostics/pathvqa_v1_loss_scaling_audit_20260924/v1_loss_scaling_audit.json}"
  local v1_eval="$PATHVQA_V1_OUTPUT_ROOT/pathvqa_v1_visual_selection_prefix_p20_norm_fixed_seed44_20260924/eval_validation/epoch_3"
  if [ ! -f "$audit_json" ] || [ ! -f "$v1_eval/pathvqa_comparisons.json" ]; then
    echo "[ERR] V3 requires the audited loss path and complete normalized V1 seed44 predictions; no training started." >&2
    return 1
  fi
  if ! python -c 'import importlib.metadata,json,sys,torch; d=json.load(open(sys.argv[1],encoding="utf-8")); assert d["passed"] is True and d["training_commit"]=="4636416ee99768c667377f0c66b5809daf48ffcd"; assert all(all(d["windows"][str(k)]["numeric_checks"].values()) for k in (16,3)); assert d["versions"]["torch"]==torch.__version__; assert d["versions"]["transformers"]==importlib.metadata.version("transformers"); assert d["versions"]["accelerate"]==importlib.metadata.version("accelerate"); print("[V3_NORM_PREFLIGHT] audit_passed=True versions_match=True full16_and_tail3=True")' "$audit_json"; then
    echo "[ERR] V3 runtime differs from audited V1 normalized path; no training started." >&2
    return 1
  fi
  local experiment_name="pathvqa_v3_postvisual_prefix_p20_norm_fixed_seed44"
  local output_dir
  mkdir -p "$PATHVQA_V3_OUTPUT_ROOT"
  output_dir="$(available_output_dir "$PATHVQA_V3_OUTPUT_ROOT" "${experiment_name}_${RUN_DATE}")"
  mkdir -p "$output_dir/eval_validation/epoch_3"
  echo "[PATHVQA_V3_CONFIG] experiment=$experiment_name git_commit=$(git -C "$ROOT_DIR" rev-parse HEAD) model_seed=44 data_seed=42 epochs=3 batch=2 accumulation=16 trainable=1864963 output=$output_dir"
  (
    cd "$ROOT_DIR" || exit 1
    python -m unittest -q test_visual_selection_postvisual || exit 1
    python -m pathvqa.train_visual_selection_postvisual \
      --model-path "$MODEL_PATH" --data-root "$PATHVQA_DATA_ROOT" \
      --cache-dir "$PATHVQA_CACHE_ROOT" --output-dir "$output_dir" \
      --experiment-name "$experiment_name" \
      2>&1 | tee "$output_dir/train.log"
  ...
  local checkpoint="$output_dir/checkpoints/epoch_3"
  [ -f "$checkpoint/visual_selection_postvisual.pt" ] || return 1
  (
    cd "$ROOT_DIR" || exit 1
    python -m pathvqa.pathvqa_official_eval \
      --backend visual-selection-postvisual --base-model "$MODEL_PATH" \
      --checkpoint "$checkpoint" --data-root "$PATHVQA_DATA_ROOT" \
      --cache-dir "$PATHVQA_CACHE_ROOT" --split validation \
      --output-dir "$output_dir/eval_validation/epoch_3" \
      2>&1 | tee "$output_dir/eval_validation_epoch_3.log"
  ...
  (
    cd "$ROOT_DIR" || exit 1
    python -m diagnostics.compare_pathvqa_v3_v1 \
      --v1-eval "$v1_eval" --v3-eval "$output_dir/eval_validation/epoch_3" \
      --output "$output_dir/paired_v1_seed44.json" \
      2>&1 | tee "$output_dir/paired_v1_seed44.log"
  ) || return 1
  echo "[PATHVQA_V3_DONE] output=$output_dir test_evaluation=false other_seeds=false"
}
...
run_pathvqa_visual_selection_offset_v0_seed44() {
  ...
  local output_dir
  output_dir="$(available_output_dir "$PATHVQA_V0_OUTPUT_ROOT" "${experiment_name}_${RUN_DATE}")"
  mkdir -p "$output_dir/eval_validation/epoch_3"
  echo "[PATHVQA_V0_CONFIG] experiment=$experiment_name model_seed=44 data_seed=42 epochs=3 split=validation expected_trainable=2356675 output=$output_dir"
  (
    cd "$ROOT_DIR" || exit 1
    python -m unittest test_visual_selection_offset \
      2>&1 | tee "$output_dir/preflight_tests.log" || exit 1
    python -m pathvqa.train_visual_selection_offset \
      --model-path "$MODEL_PATH" \
      --data-root "$PATHVQA_DATA_ROOT" \
      --output-dir "$output_dir" \
      --experiment-name "$experiment_name" \
  ...
  local checkpoint="$output_dir/checkpoints/epoch_3"
  if [ ! -f "$checkpoint/visual_selection_offset.pt" ]; then
    echo "[ERR] V0 epoch3 checkpoint missing: $checkpoint" >&2
    return 1
  fi
  (
    cd "$ROOT_DIR" || exit 1
  ...
  printf '%s\t44\tfixed_epoch3_validation\t3\t%s\t%s\n' "$experiment_name" "$score" "$checkpoint" >> "$output_dir/selected_result.tsv"
  cat "$output_dir/selected_result.tsv"
  echo "[PATHVQA_V0_DONE] output=$output_dir test_evaluation=false other_seeds=false"
}
...
run_pathvqa_v0_seed44_epoch3_diagnostic() {
  ...
  (
    ...
    python -m diagnostics.diagnose_pathvqa_v0_epoch3 \
      ...
      --data-root "$PATHVQA_DATA_ROOT" --cache-dir "$PATHVQA_CACHE_ROOT" \
      --baseline-dir "$baseline_eval" --output-dir "$output_dir" \
      2>&1 | tee "$output_dir/diagnostic.log"
  ) || return 1
...
case "$RUN_TARGET" in
  ...
  pathvqa_v1_norm_fixed_5ep_seed45)
    run_pathvqa_v1_norm_fixed_5ep_seed45 || failures=$((failures + 1))
    ;;
  ...
  pathvqa_v2_layer_mix_prefix_p20_norm_fixed_seed44)
    ...
    ;;
  pathvqa_v3_postvisual_prefix_p20_norm_fixed_seed44)
    run_pathvqa_v3_postvisual_prefix_p20_norm_fixed_seed44 || failures=$((failures + 1))
    ;;
  ...
  pathvqa_grasp_qwen_adapted_no_position_marathon10_seed44)
    ...
    ;;
  pathvqa_grasp_qwen_adapted_no_position_marathon10_lr009_seed44)
    run_pathvqa_grasp_qwen_adapted_no_position_marathon10_lr009_seed44 || failures=$((failures + 1))
    ;;
  ...
  pathvqa_qdpt_paper_figure2_training_dynamics)
    ...
    ;;
  pathvqa_qdpt_paper_seed_stability_and_dynamics)
    run_pathvqa_qdpt_paper_seed_stability_and_dynamics || failures=$((failures + 1))
    ;;
