...
run_text_guided_standard_ca_slots8_seed45() {
  SLAKE_EXPERIMENT_NAME="slake_mmrl_text_guided_standard_ca_slots8_relation0050" \
  SLAKE_RUN_SEED=45 \
  MMRL_SAME_INIT_LAYER_PROJECTORS=1 \
  MMRL_USE_DYNAMIC_CROSS_ATTENTION=1 \
...
run_text_guided_standard_ca_slots8_no_relation_seed45() {
  SLAKE_EXPERIMENT_NAME="slake_mmrl_text_guided_standard_ca_slots8_no_relation" \
  SLAKE_RUN_SEED=45 \
  MMRL_SAME_INIT_LAYER_PROJECTORS=1 \
  MMRL_USE_DYNAMIC_CROSS_ATTENTION=1 \
  MMRL_MEMORY_POOLING_MODE=text_guided \
...
case "$RUN_TARGET" in
  ...
  slake_ablation_mean_pooling_seed45)
    ...
    ;;
  slake_text_guided_standard_ca_slots8_seed45)
    run_text_guided_standard_ca_slots8_seed45 || failures=$((failures + 1))
    ;;
  slake_text_guided_standard_ca_slots8_no_relation_seed45)
    run_text_guided_standard_ca_slots8_no_relation_seed45 || failures=$((failures + 1))
    ;;
  ...
  all)
    ...
    ;;
  *)
    echo "[ERR] 未知目标: $RUN_TARGET，可选 train、slake、slake_final_seeds4、slake_ablation_no_relation_seeds2、slake_ablation_independent_init_seeds2、slake_ablation_static_query_seed45、slake_ablation_mean_pooling_seed45、slake_text_guided_standard_ca_slots8_seed45、slake_text_guided_standard_ca_slots8_no_relation_seed45、slake_ablation_suite、all。" >&2
    exit 2
    ;;
esac
