...
shutdown_on_exit() {
    ...
    echo "[EXIT] Two-experiment LoRA runner finished with exit_code=${exit_code}."
...
run_eval() {
    ...
    run_step \
        ...
        --output-dir "${experiment_dir}/eval" \
        --language all \
        --max-new-tokens 32 \
        --temperature 0 \
        --answer-mode raw \
...
printf '\n========== Two SLAKE LoRA experiments finished ==========\n'
if [[ ${#FAILED_STEPS[@]} -eq 0 ]]; then
    printf 'Both LoRA training and evaluation runs passed.\n'
    exit 0
fi
